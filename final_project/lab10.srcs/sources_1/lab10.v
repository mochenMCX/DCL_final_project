 ...
 * 42: point
 */
wire [0:7] graph_pos;  // game -> display
wire [5:0] graph;  // game -> display

wire [9:0] score;  // game -> display

wire is_started;  // game -> display
wire is_dead;  // game -> display
wire [31:0] tps;  // game -> display
...
game game(
    ...
    .graph_pos(graph_pos), .graph_out(graph),
    .score(score), .is_started(is_started), .is_dead(is_dead), .tps(tps)
);

display display(
    .clk(clk), .reset_n(reset_n), .usr_sw(usr_sw),
    .map_pos(graph_pos), .map_in(graph), .tick(tps),
